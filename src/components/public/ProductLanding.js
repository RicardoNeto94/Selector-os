...
export default function ProductLanding({ type }) {
  ...
  return <main className={`vx2 ${styles.landing}`}>
    ...
    <section className={`${styles.hero} ${type === "hospitality" ? styles.hospitalityHero : ""}`}>
      <div className={styles.heroCopy}><p className={styles.eyebrow}>{product.name} <span> / </span> {product.label}</p><h1>{product.heading}</h1><p className={styles.intro}>{product.intro}</p><div className={styles.heroActions}><a className={styles.primary} href={demoHref(product.name)} data-demo-cta={type}>Request a demo<Arrow /></a><a className={styles.textLink} href="#possibilities">Explore the possibilities<Arrow /></a></div><p className={styles.audience}>{product.audience}</p></div>
      <figure className={styles.heroVisual}><MarketingImage name={product.image} alt={product.alt} width={type === "wine" ? 1729 : 1672} height={type === "wine" ? 910 : 941} priority /><figcaption>{product.caption}</figcaption></figure>
    </section>
    ...
    <section id="access" className={styles.access}><p className={styles.eyebrow}>A personal introduction · Private pilot</p><h2>{product.cta}</h2><p>Tell us about your team and what you would like to improve. We’ll walk through the relevant features, setup and fit with you.</p><a className={styles.primary} href={demoHref(product.name)} data-demo-cta={type}>Request a demo<Arrow /></a><small>A personal introduction · No obligation</small></section>
