...
export default function ProductSections() {
  return <div id="platform" className={styles.products}>
    ...
    {Object.entries(products).map(([key, product], index) => <section id={`vaxeron-${key}`} className={`${styles.productRow} ${index ? styles.hospitality : ""}`} key={key}>
      ...
      <figure className={styles.productVisual}><MarketingImage name={product.image} alt={product.alt} width={key === "wine" ? 1729 : 1672} height={key === "wine" ? 910 : 941} /><figcaption>{product.caption}</figcaption></figure>
    </section>)}
  </div>;
}
