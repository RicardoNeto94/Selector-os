...
import { bottleQuantity, fetchAllQueryRows, positiveBottleQuantity } from "@/lib/wineInventory";
...
export default function WineInventoryPage() {
  ...
  const filteredWines = useMemo(() => {
    ...
    return wines.filter((wine) => { const haystack = [wine.name, wine.producer, wine.country, wine.region, wine.subregion, wine.grapes, wine.vintage, wine.sku].join(" ").toLowerCase(); const locationMatch = selectedLocation === "all" || wine.inventoryRows.some((row) => row.location_id === selectedLocation && (stockFilter === "out" ? bottleQuantity(row.quantity) <= 0 : positiveBottleQuantity(row.quantity) > 0)); const typeMatch = selectedType === "all" || String(wine.wine_type || "").toLowerCase() === selectedType; const stockMatch = (stockFilter === "available" && wine.stock > 0) || (stockFilter === "low" && wine.stock > 0 && wine.stock <= 2) || (stockFilter === "out" && wine.stock <= 0); return (!q || haystack.includes(q)) && locationMatch && typeMatch && stockMatch; }).sort((a, b) => { let av = sortBy === "stock" ? a.stock : a[sortBy]; let bv = sortBy === "stock" ? b.stock : b[sortBy]; if (sortBy === "price") { av = number(av); bv = number(bv); } if (typeof av === "string") av = av.toLowerCase(); if (typeof bv === "string") bv = bv.toLowerCase(); return av < bv ? (sortDirection === "asc" ? -1 : 1) : av > bv ? (sortDirection === "asc" ? 1 : -1) : 0; });
