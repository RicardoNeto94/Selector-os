...
export default function WineInventoryPage() {
  ...
  useEffect(() => {
    ...
    if (["available", "low", "negative"].includes(requestedStock)) setStockFilter(requestedStock);
  ...
  return <div className="wine-inventory-page min-h-screen"><div className="wine-inventory-shell mx-auto max-w-[1700px] px-5 py-7 md:px-8 lg:px-10">
    ...
    {loading ? <section className="inventory-loading-overview mt-5" aria-live="polite"><ArrowPathIcon className="animate-spin" /><div><strong>Reading current inventory</strong><span>Loading balances, locations and the latest Compucash status…</span></div></section> : <>
      ...
      <details className="inventory-secondary-details mt-4" open={selectedFormat !== "all"}>
        <summary><div><span>Packaging detail</span><strong>Bottle formats and open fractions</strong></div><div>{selectedFormat === "all" ? "Open formats" : `${BOTTLE_FORMATS[selectedFormat]?.label || "Filtered"} selected`} <ChevronDownIcon /></div></summary>
        <section className="inventory-format-grid" aria-label="Stock by bottle format">
