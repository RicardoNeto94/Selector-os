...
export default function WineInventoryPage() {
  ...
  const unopenedBottles = useMemo(() => sumWholeBottles(inventory, (row) => row.quantity), [inventory]);
  ...
  return <div className="wine-inventory-page min-h-screen"><div className="wine-inventory-shell mx-auto max-w-[1700px] px-5 py-7 md:px-8 lg:px-10">
    ...
    <section className="inventory-kpi-grid mt-5 grid grid-cols-2 overflow-hidden lg:grid-cols-4"><Kpi label="Unopened bottles" value={quantity(unopenedBottles)} detail={`Whole positive balances across ${locations.length} locations · ${quantity(bottleFormats.fractional)} open equivalents excluded`} /><Kpi label="Wine physical units" value={quantity(inventoryFamilies.wine.positive)} detail="Includes open wine fractions · excludes sake and alcohol-free" /><Kpi label="Sake & shochu units" value={quantity(inventoryFamilies.sake.positive)} detail="Includes open sake and shochu fractions" /><Kpi label="Active labels" value={metrics.available.toLocaleString("en-GB")} detail="Unique products with stock above zero" active={stockFilter === "available"} onClick={() => setStockFilter("available")} tone="good" /></section>
    ...
    {selectedFormat === "unknown" && <SizeReviewPanel wines={wines.filter((wine) => hasAvailableStock(wine.stock) && bottleFormatForWine(wine) === "unknown")} savingSizeId={savingSizeId} onSave={saveWineSize} />}
