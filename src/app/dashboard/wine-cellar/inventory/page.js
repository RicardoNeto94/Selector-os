...
export default function WineInventoryPage() {
  ...
  useEffect(() => {
    ...
    if (["available", "low", "out"].includes(requestedStock)) setStockFilter(requestedStock);
  ...
  const filteredWines = useMemo(() => {
    ...
    return wines.filter((wine) => { const haystack = [wine.name, wine.producer, wine.country, wine.region, wine.subregion, wine.grapes, wine.vintage, wine.sku].join(" ").toLowerCase(); const locationMatch = selectedLocation === "all" || wine.inventoryRows.some((row) => row.location_id === selectedLocation && (stockFilter === "out" ? bottleQuantity(row.quantity) <= 0 : positiveBottleQuantity(row.quantity) > 0)); const typeMatch = selectedType === "all" || String(wine.wine_type || "").toLowerCase() === selectedType; const formatMatch = selectedFormat === "all" || bottleFormatForWine(wine) === selectedFormat; const stockMatch = (stockFilter === "available" && wine.stock > 0) || (stockFilter === "low" && wine.stock > 0 && wine.stock <= 2) || (stockFilter === "out" && wine.stock <= 0); return (!q || haystack.includes(q)) && locationMatch && typeMatch && formatMatch && stockMatch; }).sort((a, b) => { let av = sortBy === "stock" ? a.stock : a[sortBy]; let bv = sortBy === "stock" ? b.stock : b[sortBy]; if (sortBy === "price") { av = number(av); bv = number(bv); } if (typeof av === "string") av = av.toLowerCase(); if (typeof bv === "string") bv = bv.toLowerCase(); return av < bv ? (sortDirection === "asc" ? -1 : 1) : av > bv ? (sortDirection === "asc" ? 1 : -1) : 0; });
  ...
  return <div className="wine-inventory-page min-h-screen"><div className="wine-inventory-shell mx-auto max-w-[1700px] px-5 py-7 md:px-8 lg:px-10">
    ...
    <section className="inventory-table-card mt-5 overflow-hidden"><div className="inventory-filter-panel p-4"><div className="inventory-filter-grid grid gap-3 xl:grid-cols-[1fr_180px_210px_180px_auto]"><label className="inventory-search-field"><span className="mb-1.5 block text-[8px] uppercase tracking-[0.16em] text-[#71877f]">Search {stockFilter === "out" ? "ordering list" : "available inventory"}</span><input aria-label="Search catalogue" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Wine, producer, region, vintage or SKU…" className="min-h-11 w-full rounded-xl px-4 text-[11px] outline-none" /></label><Filter label="Wine type" value={selectedType} onChange={setSelectedType}>{TYPES.map((type) => <option key={type} value={type}>{type === "all" ? "All types" : type}</option>)}</Filter><Filter label="Location" value={selectedLocation} onChange={setSelectedLocation}><option value="all">All locations</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</Filter><Filter label="Stock view" value={stockFilter} onChange={setStockFilter}><option value="available">Available stock</option><option value="low">Low stock ≤ 2</option><option value="out">Needs ordering · zero stock</option></Filter><button type="button" onClick={() => { setSearch(""); setSelectedType("all"); setSelectedLocation("all"); setStockFilter("available"); }} className="inventory-reset self-end px-4 text-[8px] uppercase tracking-[0.18em]">Reset filters</button></div></div>
