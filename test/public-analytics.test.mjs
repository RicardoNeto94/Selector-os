...
test("both public events retain PostHog's required ingestion token and anonymous ID", () => {
  for (const event of ["$pageview", "demo_interest"]) {
    const result = filterPublicAnalyticsEvent({
      ...
      properties: { token: "public-project-token", distinct_id: "anonymous-id", path: "/wine", $host: "vaxeron.com", $pathname: "/wine", product: "Wine", $session_id: "anonymous-session", $window_id: "anonymous-window", $process_person_profile: false },
    });
    ...
    assert.equal(result.properties.$host, "vaxeron.com");
    assert.equal(result.properties.$pathname, "/wine");
